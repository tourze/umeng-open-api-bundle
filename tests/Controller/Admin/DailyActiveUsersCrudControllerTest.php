...
use UmengOpenApiBundle\Controller\Admin\DailyActiveUsersCrudController;

/**
 * DailyActiveUsersCrudController Web测试
 *
 * @internal
 */
#[CoversClass(DailyActiveUsersCrudController::class)]
#[RunTestsInSeparateProcesses]
final class DailyActiveUsersCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    /** @phpstan-return DailyActiveUsersCrudController */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(DailyActiveUsersCrudController::class);
    }
    ...
    {
        $reflection = new \ReflectionClass(DailyActiveUsersCrudController::class);

        $this->assertTrue($reflection->isInstantiable());
        $this->assertTrue($reflection->isFinal());
    }

    public function testGetEntityFqcnShouldReturnDailyActiveUsersClass(): void
    {
        $this->assertEquals('UmengOpenApiBundle\Entity\DailyActiveUsers', DailyActiveUsersCrudController::getEntityFqcn());
    }
    ...
    public function testControllerHasProperNamespace(): void
    {
        $this->assertEquals(
            ...
            (new \ReflectionClass(DailyActiveUsersCrudController::class))->getNamespaceName()
        );
    }
    ...
    public function testValidationErrors(): void
    {
        ...
        // 提交空表单
        $form = $crawler->filter('form[name="DailyActiveUsers"]')->form();
        $client->submit($form);

        // 验证返回422状态码
        $this->assertResponseStatusCodeSame(422);

        // 验证响应内容包含必填字段错误信息（日期字段使用NotNull约束）

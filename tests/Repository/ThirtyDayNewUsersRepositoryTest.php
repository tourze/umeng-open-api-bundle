 ...
 * @internal
 */
#[CoversClass(ThirtyDayNewUsersRepository::class)]
#[RunTestsInSeparateProcesses]
final class ThirtyDayNewUsersRepositoryTest extends AbstractRepositoryTestCase
{
    private ThirtyDayNewUsersRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(ThirtyDayNewUsersRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(ThirtyDayNewUsersRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createThirtyDayNewUsers($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createThirtyDayNewUsers($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\ThirtyDayNewUsers', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createThirtyDayNewUsers($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createThirtyDayNewUsers($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    public function testSaveMethodShouldPersistEntity(): void
    {
        ...
        $this->assertInstanceOf('UmengOpenApiBundle\Entity\ThirtyDayNewUsers', $savedData);
    ...
    public function testRemoveMethodShouldDeleteEntity(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createThirtyDayNewUsers($app, new \DateTimeImmutable('2024-04-15'), 100);
        $dataId = $data->getId();
    ...
    public function testRemoveMethodWithFlushFalseShouldNotFlushImmediately(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createThirtyDayNewUsers($app, new \DateTimeImmutable('2024-05-15'), 200);
        $dataId = $data->getId();
    ...
    private function createThirtyDayNewUsers(App $app, \DateTimeInterface $date, int $value): ThirtyDayNewUsers
    {
        $data = new ThirtyDayNewUsers();
        $data->setApp($app);
        $data->setDate($date);
    ...
    protected function getRepository(): ThirtyDayNewUsersRepository
    {

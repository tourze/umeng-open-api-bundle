 ...
 * @internal
 */
#[CoversClass(ThirtyDayLaunchesRepository::class)]
#[RunTestsInSeparateProcesses]
final class ThirtyDayLaunchesRepositoryTest extends AbstractRepositoryTestCase
{
    private ThirtyDayLaunchesRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(ThirtyDayLaunchesRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(ThirtyDayLaunchesRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createThirtyDayLaunches($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createThirtyDayLaunches($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\ThirtyDayLaunches', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createThirtyDayLaunches($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createThirtyDayLaunches($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createThirtyDayLaunches(App $app, \DateTimeInterface $date, int $value): ThirtyDayLaunches
    {
        $data = new ThirtyDayLaunches();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): ThirtyDayLaunchesRepository
    {

 ...
 * @internal
 */
#[CoversClass(SevenDaysNewUsersRepository::class)]
#[RunTestsInSeparateProcesses]
final class SevenDaysNewUsersRepositoryTest extends AbstractRepositoryTestCase
{
    private SevenDaysNewUsersRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(SevenDaysNewUsersRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(SevenDaysNewUsersRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createSevenDaysNewUsers($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysNewUsers($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\SevenDaysNewUsers', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createSevenDaysNewUsers($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysNewUsers($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createSevenDaysNewUsers(App $app, \DateTimeInterface $date, int $value): SevenDaysNewUsers
    {
        $data = new SevenDaysNewUsers();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): SevenDaysNewUsersRepository
    {

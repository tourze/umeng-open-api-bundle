 ...
 * @internal
 */
#[CoversClass(WeeklyNewUsersRepository::class)]
#[RunTestsInSeparateProcesses]
final class WeeklyNewUsersRepositoryTest extends AbstractRepositoryTestCase
{
    private WeeklyNewUsersRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(WeeklyNewUsersRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(WeeklyNewUsersRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createWeeklyNewUsers($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyNewUsers($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\WeeklyNewUsers', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createWeeklyNewUsers($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyNewUsers($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createWeeklyNewUsers(App $app, \DateTimeInterface $date, int $value): WeeklyNewUsers
    {
        $data = new WeeklyNewUsers();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): WeeklyNewUsersRepository
    {

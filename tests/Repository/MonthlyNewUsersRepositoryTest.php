 ...
 * @internal
 */
#[CoversClass(MonthlyNewUsersRepository::class)]
#[RunTestsInSeparateProcesses]
final class MonthlyNewUsersRepositoryTest extends AbstractRepositoryTestCase
{
    private MonthlyNewUsersRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(MonthlyNewUsersRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(MonthlyNewUsersRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createMonthlyNewUsers($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createMonthlyNewUsers($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\MonthlyNewUsers', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createMonthlyNewUsers($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createMonthlyNewUsers($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createMonthlyNewUsers(App $app, \DateTimeInterface $date, int $value): MonthlyNewUsers
    {
        $data = new MonthlyNewUsers();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): MonthlyNewUsersRepository
    {

 ...
 * @internal
 */
#[CoversClass(SevenDaysLaunchesRepository::class)]
#[RunTestsInSeparateProcesses]
final class SevenDaysLaunchesRepositoryTest extends AbstractRepositoryTestCase
{
    private SevenDaysLaunchesRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(SevenDaysLaunchesRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(SevenDaysLaunchesRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createSevenDaysLaunches($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysLaunches($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\SevenDaysLaunches', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createSevenDaysLaunches($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysLaunches($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createSevenDaysLaunches(App $app, \DateTimeInterface $date, int $value): SevenDaysLaunches
    {
        $data = new SevenDaysLaunches();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): SevenDaysLaunchesRepository
    {

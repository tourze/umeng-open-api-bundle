 ...
 * @internal
 */
#[CoversClass(DailyRetentionsRepository::class)]
#[RunTestsInSeparateProcesses]
final class DailyRetentionsRepositoryTest extends AbstractRepositoryTestCase
{
    private DailyRetentionsRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(DailyRetentionsRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(DailyRetentionsRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createDailyRetentions($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createDailyRetentions($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\DailyRetentions', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createDailyRetentions($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createDailyRetentions($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    public function testSaveMethodShouldPersistEntity(): void
    {
        ...
        $this->assertInstanceOf('UmengOpenApiBundle\Entity\DailyRetentions', $savedData);
    ...
    public function testRemoveMethodShouldDeleteEntity(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createDailyRetentions($app, new \DateTimeImmutable('2024-04-15'), 100);
        $dataId = $data->getId();
    ...
    public function testRemoveMethodWithFlushFalseShouldNotFlushImmediately(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createDailyRetentions($app, new \DateTimeImmutable('2024-05-15'), 200);
        $dataId = $data->getId();
    ...
    private function createDailyRetentions(App $app, \DateTimeInterface $date, int $value): DailyRetentions
    {
        $data = new DailyRetentions();
        $data->setApp($app);
        $data->setDate($date);
    ...
    protected function createNewEntity(): object
    {
        ...
        $entity->setRetentionRate(['day1' => 0.8, 'day7' => 0.5]);

        return $entity;
    }

    protected function getRepository(): DailyRetentionsRepository
    {

 ...
 * @internal
 */
#[CoversClass(SevenDaysActiveUsersRepository::class)]
#[RunTestsInSeparateProcesses]
final class SevenDaysActiveUsersRepositoryTest extends AbstractRepositoryTestCase
{
    private SevenDaysActiveUsersRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(SevenDaysActiveUsersRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(SevenDaysActiveUsersRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createSevenDaysActiveUsers($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysActiveUsers($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\SevenDaysActiveUsers', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createSevenDaysActiveUsers($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createSevenDaysActiveUsers($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    public function testSaveMethodShouldPersistEntity(): void
    {
        ...
        $this->assertInstanceOf('UmengOpenApiBundle\Entity\SevenDaysActiveUsers', $savedData);
    ...
    public function testRemoveMethodShouldDeleteEntity(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createSevenDaysActiveUsers($app, new \DateTimeImmutable('2024-04-15'), 100);
        $dataId = $data->getId();
    ...
    public function testRemoveMethodWithFlushFalseShouldNotFlushImmediately(): void
    {
        $account = $this->createAccount('测试账户', 'test_api_key', 'test_security');
        $app = $this->createApp('测试应用', 'test_app_key', 'iOS', $account);
        $data = $this->createSevenDaysActiveUsers($app, new \DateTimeImmutable('2024-05-15'), 200);
        $dataId = $data->getId();
    ...
    private function createSevenDaysActiveUsers(App $app, \DateTimeInterface $date, int $value): SevenDaysActiveUsers
    {
        $data = new SevenDaysActiveUsers();
        $data->setApp($app);
        $data->setDate($date);
    ...
    protected function getRepository(): SevenDaysActiveUsersRepository
    {

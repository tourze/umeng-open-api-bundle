 ...
 * @internal
 */
#[CoversClass(WeeklyLaunchesRepository::class)]
#[RunTestsInSeparateProcesses]
final class WeeklyLaunchesRepositoryTest extends AbstractRepositoryTestCase
{
    private WeeklyLaunchesRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(WeeklyLaunchesRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(WeeklyLaunchesRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createWeeklyLaunches($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyLaunches($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\WeeklyLaunches', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createWeeklyLaunches($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyLaunches($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createWeeklyLaunches(App $app, \DateTimeInterface $date, int $value): WeeklyLaunches
    {
        $data = new WeeklyLaunches();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): WeeklyLaunchesRepository
    {

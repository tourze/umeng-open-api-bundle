 ...
 * @internal
 */
#[CoversClass(WeeklyRetentionsRepository::class)]
#[RunTestsInSeparateProcesses]
final class WeeklyRetentionsRepositoryTest extends AbstractRepositoryTestCase
{
    private WeeklyRetentionsRepository $repository;

    protected function onSetUp(): void
    {
        $this->repository = self::getService(WeeklyRetentionsRepository::class);
    }

    public function testRepository(): void
    {
        $this->assertInstanceOf(WeeklyRetentionsRepository::class, $this->repository);
    }
    ...
    public function testFindAllWhenRecordsExistShouldReturnEntities(): void
    {
        ...
        $data1 = $this->createWeeklyRetentions($app, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyRetentions($app, new \DateTimeImmutable('2024-01-02'), 1100);

        $results = $this->repository->findAll();

        $this->assertIsArray($results);
        $this->assertGreaterThanOrEqual(2, count($results));
        $this->assertContainsOnlyInstancesOf('UmengOpenApiBundle\Entity\WeeklyRetentions', $results);
    }
    ...
    public function testFindByAppAssociationShouldReturnRelatedData(): void
    {
        ...
        $data1 = $this->createWeeklyRetentions($app1, new \DateTimeImmutable('2024-01-01'), 1000);
        $data2 = $this->createWeeklyRetentions($app2, new \DateTimeImmutable('2024-01-01'), 2000);
    ...
    private function createWeeklyRetentions(App $app, \DateTimeInterface $date, int $value): WeeklyRetentions
    {
        $data = new WeeklyRetentions();
        $data->setApp($app);
        $data->setDate($date);
        $data->setValue($value);
    ...
    protected function getRepository(): WeeklyRetentionsRepository
    {

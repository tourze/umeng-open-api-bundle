...
#[AsCommand(name: self::NAME, description: '获取App新增用户数(月)')]
final class GetMonthlyNewUsersCommand extends Command
{
    ...
    private function processAppNewUsers(App $app, array $dateRange): void
    {
        ...
        $result = $this->dataFetcher->fetchMonthlyNewUsers($app, $startDate, $endDate);
    ...
    private function saveNewUsersData(App $app, \UmengUappGetNewUsersResult $result): void
    {
        ...
        foreach ($newUserInfo as $item) {
            ...
            if (null === $dbItem) {
                $dbItem = new MonthlyNewUsers();
                $dbItem->setApp($app);
                $dbItem->setDate($date);
            }
            /** @var MonthlyNewUsers $dbItem */
